...
nodeCanvasObject={(node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
  ...
  // Define zoom threshold for LOD
  const DETAIL_THRESHOLD = 5.5; // Adjust this value to change when the switch happens
  
  if (globalScale < DETAIL_THRESHOLD) {
    // OPTION 1: Circle/Pie Chart Nodes (Zoomed Out)
    ...
    // OPTION 2: Image Nodes (Zoomed In)
  ...
  // Optional: Adjust label size based on zoom level
  const fontSize = Math.min(12/globalScale, 16); // Prevent text from getting too large
  ctx.font = `${fontSize}px Sans-Serif`;
  const maxWidth = 100/globalScale;
  
  // Calculate label position based on zoom level and node type
  const getLabelY = () => {
    if (globalScale < DETAIL_THRESHOLD) {
      // Circle mode - position below the circle
      return y + radius + (fontSize/2);
    } else {
      // Card mode - position below the card image
      const cardAspectRatio = 63/88;
      const cardWidth = size * .8;
      const cardHeight = cardWidth / cardAspectRatio;
      return y + (cardHeight/2) + (fontSize/2);
    }
  };

  const textY = getLabelY();
  
  // Rest of the label rendering code...
