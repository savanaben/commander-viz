...
for i in range(len(commanders)):
 ...
 # Add debug check here
        ...
        if commander_data[cmd1] and commander_data[cmd2]:
            ...
            normalized_raw_weight = normalize_raw_weight(total_raw_overlap, max_possible_overlap)  # Simple deck size normalization
            total_normalized_overlap = sum(normalized_overlaps.values()) / len(CARD_CATEGORIES)    # Color-identity normalized
            avg_uniqueness = sum(uniqueness_scores.values()) / len(CARD_CATEGORIES)               # Card uniqueness
...
# normalizing tribe weight
print("Normalizing tribe weights...")
# Collect all non-zero tribe weights
tribe_weights = [edge['tribes_weight'] for edge in edge_data if edge['tribes_weight'] > 0]

if tribe_weights:
    print("\nBefore normalization:")
    weight_distribution = {
        "0.0-0.1": len([w for w in tribe_weights if w <= 0.1]),
        "0.1-0.2": len([w for w in tribe_weights if 0.1 < w <= 0.2]),
        "0.2-0.3": len([w for w in tribe_weights if 0.2 < w <= 0.3]),
        "0.3-0.4": len([w for w in tribe_weights if 0.3 < w <= 0.4]),
        "0.4+": len([w for w in tribe_weights if w > 0.4])
    }
    ...
    # Apply more aggressive transformation to spread out the values
    # not really proven to be fair/work well
    for edge in edge_data:
        if edge['tribes_weight'] > 0:
            # More aggressive transformation:
            # 1. Scale up small values more dramatically (multiply by 20 instead of 9)
            # 2. Use square root to spread values more evenly
            # 3. Then normalize to 0-1 range
            boosted_weight = edge['tribes_weight'] * 20
            transformed = math.sqrt(boosted_weight)
            edge['tribes_weight'] = round(
                min(transformed, 1.0),  # Cap at 1.0
                3
            )

    # Print distribution after transformation
    transformed_weights = [edge['tribes_weight'] for edge in edge_data if edge['tribes_weight'] > 0]
    print("\nAfter log transformation:")
    weight_distribution = {
        "0.0-0.2": len([w for w in transformed_weights if w <= 0.2]),
        "0.2-0.4": len([w for w in transformed_weights if 0.2 < w <= 0.4]),
        "0.4-0.6": len([w for w in transformed_weights if 0.4 < w <= 0.6]),
        "0.6-0.8": len([w for w in transformed_weights if 0.6 < w <= 0.8]),
        "0.8-1.0": len([w for w in transformed_weights if w > 0.8])
    }
    print("Weight distribution:", weight_distribution)
    print(f"Min weight: {min(transformed_weights):.3f}")
    print(f"Max weight: {max(transformed_weights):.3f}")
